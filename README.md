...
基于 FFmpeg 7.0.2 和 SDL2 2.30.6 的多媒体播放器实践项目，包含音频播放、PCM 播放、视频播放、多线程音视频同步播放器（Windows/WSL 双版本）以及 HW 硬件加速解码的音视频播放器（有无滤镜版本），一共六个独立示例。
...
├── Audio_Video_Player(WSL).cpp   # WSL 版本音视频播放器源代码
...
- 引入滤镜库，支持对不同分辨率和横竖方向的视频播放做出旋转伸缩适配
...
![硬件加速全过程](Pictures/HW_accelerate.png)
...
**差异版本**: 最新版本的 **HW_Audio_Video_Player.cpp** 在 **HW.cpp** 的基础上新增引入了滤镜库支持对不同分辨率和横竖方向的视频播放做出旋转伸缩适配，此外减少了线程数量提高单线程效率，还优化了大量内存管理与回退机制，播放同一高压视频的内存占用仅是原先的一半！
...
### v1.3.0 (2026.3.8)
- **新增**：`Audio_Video_Player.cpp` `HW_Audio_Video_Player.cpp`均引入 FFmpeg 滤镜库，保留原先未引入滤镜库的硬件加速播放器代码为`HW.cpp`，便于以后差异化学习以及性能比较。
